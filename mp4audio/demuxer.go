package mp4audio

import (
	"errors"
	"io"

	mp4 "github.com/abema/go-mp4"
	"github.com/sunfish-shogi/bufseekio"
)

type MP4Audio struct {
	MdatOffset uint64

	reader     io.ReadSeeker
	track      *mp4.Track
	trackIndex int
}

func New(reader io.ReadSeeker) (*MP4Audio, error) {
	bufferedReader := bufseekio.NewReadSeeker(reader, 256*256, 4)
	audioTrack, err := getAudioTrack(bufferedReader)
	if err != nil {
		return nil, err
	}

	mdatOffset, err := getMdatOffset(bufferedReader)
	if err != nil {
		return nil, err
	}

	return &MP4Audio{
		reader:     reader,
		track:      audioTrack,
		trackIndex: int(audioTrack.TrackID) - 1,
		MdatOffset: mdatOffset,
	}, nil
}

// フレーム情報のイテレータを生成する
func (mp4audio *MP4Audio) Frames() (*FrameIterator, error) {
	return &FrameIterator{
		samples: mp4audio.track.Samples,
		index:   0,
	}, nil
}

type FrameIterator struct {
	samples mp4.Samples
	index   int
}

// イテレーションしてフレームの情報を返すメソッド
func (v *FrameIterator) Next() *mp4.Sample {
	if len(v.samples) <= v.index {
		return nil
	}

	v.index++
	return v.samples[v.index-1]
}

// esdsからASCの値を含むデスクリプタを取り出す
func (mp4audio *MP4Audio) ASCDescriptor() (*mp4.Descriptor, error) {
	results, err := mp4.ExtractBoxWithPayload(mp4audio.reader, nil, mp4.BoxPath{
		mp4.BoxTypeMoov(),
		mp4.BoxTypeTrak(),
		mp4.BoxTypeMdia(),
		mp4.BoxTypeMinf(),
		mp4.BoxTypeStbl(),
		mp4.BoxTypeStsd(),
		mp4.BoxTypeMp4a(),
		mp4.BoxTypeEsds(),
	})
	if err != nil {
		return nil, err
	}

	// audioトラックに該当するesdsが無いのはおかしい
	if len(results) < mp4audio.trackIndex {
		return nil, errors.New("no esds atom available")
	}
	esds := results[mp4audio.trackIndex].Payload.(*mp4.Esds)

	var ascDescriptor *mp4.Descriptor
	for _, descriptor := range esds.Descriptors {
		if descriptor.Tag == mp4.DecSpecificInfoTag {
			ascDescriptor = &descriptor
			break
		}
	}

	if ascDescriptor == nil {
		return nil, errors.New("no descriptor found")
	}

	return ascDescriptor, nil
}

// mdatのオフセット(bytes)を取得する
func getMdatOffset(reader io.ReadSeeker) (uint64, error) {
	results, err := mp4.ExtractBox(reader, nil, mp4.BoxPath{mp4.BoxTypeMdat()})
	if err != nil {
		return 0, err
	}

	// mdatは常にひとつしかないはずなので0じゃなければいいとする
	if len(results) == 0 {
		return 0, errors.New("no mdat atom available")
	}
	stream := results[0]

	// mdatのオフセットからさらにメタデータを含むヘッダサイズ分を飛ばして
	// 実際のメディアデータのバイナリが始まる位置をmdatOffsetとする
	return stream.Offset + stream.HeaderSize, nil
}

// オーディオのstreamを取得する
func getAudioTrack(reader io.ReadSeeker) (*mp4.Track, error) {
	info, err := mp4.Probe(reader)
	if err != nil {
		return nil, err
	}

	var targetTrack *mp4.Track
	for _, track := range info.Tracks {
		if track.Codec == mp4.CodecMP4A {
			targetTrack = track
			break
		}
	}
	if targetTrack == nil {
		return nil, errors.New("no audio track available")
	}

	return targetTrack, nil
}
